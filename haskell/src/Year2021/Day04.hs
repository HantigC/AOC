...
toBingoBoard =
  map $ map (\x -> (read x, False)) . filter (/= "") . S.splitOn " "
...
playBingo
  :: (Int -> [BingoBoard] -> ([BingoBoard], Maybe BingoBoard))
  -> BingoGame
  -> Winner
playBingo strategy (seq, boards) = playNext seq boards
 where
  playNext []       boards = Nothing
  playNext (s : ss) boards = case strategy s boards of
    (updatedBoards, Nothing) -> playNext ss updatedBoards
    (_, Just board) ->
      Just $ (* s) $ sum . map (sum . map fst . filter (not . snd)) $ board
...
updateBoard num =
  map $ map (\cell@(num', _) -> if num' == num then (num, True) else cell)
...
checkBoard = doCheck []
 where
  ...
  doCheck prev (b : bs) | and checkedRow = True
...
winOnFirst s boards = (updatedBoards, getFirstWinBoard updatedBoards)
 where
  ...
  getFirstWinBoard (b : bs) | checkBoard b = Just b
...
main = do
  lines <- U.readLines $ U.get2021Resources 4
