...
partOne lines = sum . map (\k -> M.findWithDefault 0 k mm) $ [2, 3, 4, 7]
  where
    mm = U.count . map length . concat . get4Nums $ lines
...
findNumbers sevenSegments = do
  ...
  (three, fiveLengths) <-
    oneAndRest
      (sizeEqTo 2 . (one `Set.intersection`))
      fiveLengths
  let zeroPattern = eight `Set.difference` three `Set.union` one

  (zero, sixLengths) <-
    oneAndRest
      (sizeEqTo 4 . Set.intersection zeroPattern)
      sixLengths
  (nine, sixLengths) <-
    oneAndRest
      (sizeEqTo 2 . Set.intersection one)
      sixLengths
...
partTwo ls = do
  ...
  where
    f x = do
      ...
      where
        (searchSpace : matchSpace : _) = map words . S.splitOn " | " $ x
