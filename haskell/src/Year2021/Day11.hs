...
smallEnergyLevels = ["11111", "19991", "19191", "19991", "11111"]
...
computeFlashNumber num grid = snd $ foldl f (grid, 0) [1 .. num]
  where
    f (grid, flashes) _ = (A.amap f grid', flashes + flashes')
      where
        grid' = fillOneStep grid
...
bfs grid (coord@(y, x) : coords) = f el
  where
    ...
    f (Level x)
      ...
      where
        neighCoords = U.getSquareNeighbCoords coord grid
...
fillOneStep energyLevels = energyLevels'
  where
    ...
    energyLevels' =
      bfs energyLevels [(i, j) | i <- [sh .. eh], j <- [sw .. ew]]

partOne :: TextList -> Int
partOne strs = computeFlashNumber 100 energyLevels
  where
    energyLevels = textListToArray f strs
    ((sh, sw), (eh, ew)) = A.bounds energyLevels
...
main :: IO ()
main = do
  lines <- U.readLines $ U.get2021Resources 11
