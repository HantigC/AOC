...
polymerization ::
  ...
  M.Map String Char ->
  (M.Map String Int, M.Map Char Int)
polymerization (polymerMap, charCnt) expansionMap =
  M.foldrWithKey
    f
    (M.empty, charCnt)
    polymerMap
  where
    f (x1 : x2 : _) n (mapp, cntMap) =
      ( M.insertWith (+) [x1, xm] n (M.insertWith (+) [xm, x2] n mapp),
        M.insertWith (+) xm n cntMap
      )
      where
        xm = expansionMap M.! [x1, x2]
...
polymerizations ::
  ...
  Int ->
  (M.Map String Int, M.Map Char Int)
polymerizations polymerMap expansionMap n =
  foldl (\acc _ -> polymerization acc expansionMap) polymerMap [1 .. n]

countElements :: Ord a => [a] -> M.Map a Int
countElements = foldl f M.empty where f mapp chr = M.insertWith (+) chr 1 mapp

partOne :: String -> Int -> Either ParseError Int
partOne str n = do
  (expansionMap, polymerTemplate) <- parse parsePolymerization "" str
  let (s, cnts) =
        polymerizations (polymerMap, countElements polymerTemplate) expansionMap n
      polymerMap =
        M.fromListWith (+) $ zip (U.convolve polymerTemplate 2) (repeat 1)
      minCnt = minimum $ M.elems cnts
      maxCnt = maximum $ M.elems cnts
  return $ maxCnt - minCnt

main = do
  input <- readFile $ U.get2021Resources 14
