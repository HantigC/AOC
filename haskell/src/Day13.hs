...
data TransparentPaper = TransparentPaper { paperStensil :: [(Int, Int)]
                                         , folds :: [Fold]
                                         }
...
parseFold = do
  ...
  where parseX = string "x=" >> return FoldX
...
foldTransparentPaper coords foldType =
  case foldType of
    (FoldX foldX) -> nub . map (\(x,y) -> (f x foldX maxX, y)) . filter ((/= foldX) . fst) $ coords
    (FoldY foldY) -> nub . map (\(x,y) -> (x, f y foldY maxY)) . filter ((/= foldY) . snd) $ coords
  where f coord foldCoord maxCoord
          | coord < foldCoord = coord
...
partTwo transparentPaper  = case transparentPaper of
    (Right (TransparentPaper paper folds)) -> showPaper $ foldl foldTransparentPaper paper folds
    (Left _) -> ""


main = do
  resource <- readFile "../resources/Day13.txt"
