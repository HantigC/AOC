...
checkSyntax = map (f [])
  where f [] [] = Ok
        ...
        f stack (s@'{':str) = f (s:stack) str
        f stack (s@'<':str) = f (s:stack) str
        f (ss:stack) (s@'>':str) = if ss == '<' then f stack str else SyntaxError s
        f (ss:stack) (s@']':str) = if ss == '[' then f stack str else SyntaxError s
        f (ss:stack) (s@'}':str) = if ss == '{' then f stack str else SyntaxError s
        f (ss:stack) (s@')':str) = if ss == '(' then f stack str else SyntaxError s
...
partOne strs = sum <$> mapM getScore errornousSyntaxes
  where isSyntaxError (SyntaxError _) = True
...
partTwo strs = do
  ...
  where isSyntaxIncomplete (SyntaxIncomplete _) = True
