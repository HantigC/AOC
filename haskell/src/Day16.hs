...
data BITS = LiteralValue { version :: Int , value :: Int , packType :: Int}
          | Empty String Int
    ...
    packType -> do
      ...
      case oneZero of
        '0' -> do
          ...
          return $ Operator (length packages) (U.binToDecStr packageVersion) packages (U.binToDecStr packType) 0
        _ -> do
          elevenBits <- count 11 digit
          packages <- count (U.binToDecStr elevenBits) parsePackage
          return $ Operator (length packages) (U.binToDecStr packageVersion) packages (U.binToDecStr packType) 1
  where parseNext n = do
          pos <- sourceColumn <$> getPosition
...
sumVersions (Operator _ version subpackes _ _) = version + (sum . map sumVersions $ subpackes)
...
compute (Operator _ _ [p1, p2] 5 _) = boolToInt <$> ((>) <$> compute p1 <*> compute p2)
compute (Operator _ _ [p1, p2] 6 _) = boolToInt <$> ((<) <$> compute p1 <*> compute p2)
compute (Operator _ _ [p1, p2] 7 _) = boolToInt <$> ((==) <$> compute p1 <*> compute p2)
compute _  = Nothing
...
main = do
  ...
  input <- readFile "../resources/Day16.txt"
  print $ partOne input
  print $ partTwo input
