...
partition criteria = foldr select ([], [])
 where
  select x (xs, ys) | criteria x = (x : xs, ys)
                    | otherwise  = (xs, x : ys)
...
partOne xn@(x : xs) = o2 * co2
 where
  ...
  countBits (bits, cnt) strBits =
    (zipWith (+) bits $ map toBit strBits, cnt + 1)

  compMajority :: Int -> Int
  compMajority x =
    if fromIntegral x / fromIntegral totalCnt > 0.5 then 1 else 0
...
partTwo xs = bitStrToDec o2 * bitStrToDec co2
 where
  ...
  selectBy f ith xs  = selectBy f (ith + 1) $ f zeros ones
    where (zeros, ones) = partition (\str -> str !! ith == '0') xs
...
main = do
  bitStr <- U.readLines $ U.get2021Resources 3
