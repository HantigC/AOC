...
findSmokeBasin arr = sequence
  $ filter (/= Nothing) [ f (y, x) | y <- [0 .. h], x <- [0 .. w] ]
 where
  bounds@(_, (h, w)) = A.bounds arr
  getIfInBounds coord | U.inBounds bounds coord = Just $ arr A.! coord
                      | otherwise               = Nothing
...
partTwo strBasins = do
  bazins <- smokeBasins
  return $ product . take 3 . reverse . sort . map length $ extractBasins
    bazins
    mapp
    visitedMask
 where
