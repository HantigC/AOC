...
instance Read Venture where
  readsPrec _ s =
    [(Venture { st = (read x1, read y1), nd = (read x2, read y2) }, "")]
   where
    ((x1 : y1 : _) : (x2 : y2 : _) : _) =
      map (S.splitOn ",") . S.splitOn " -> " $ s


play :: [Venture] -> Int
play = length . M.filter (> 1) . foldl store M.empty
  where store m v = M.unionWith (+) m . M.fromList . zip (expand v) $ repeat 1
...
main :: IO ()
main = do
  lines <- U.readLines $ U.get2021Resources 5
  print "First"
