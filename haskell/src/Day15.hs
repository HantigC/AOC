...
findLowestRiskPath grid = f (h, w)
  where (_, (h, w)) = A.bounds grid
        ...
        f (y, x) = grid A.! (y, x) + minimum [ fm (y-1, x)
                                             , fm (y, x-1)
                                             ]

data CellCost = CellCost U.Coord Int deriving (Eq, Show)

instance Ord CellCost where
  (CellCost _ c1) < (CellCost _ c2) = c1 < c2
  (CellCost _ c1) <= (CellCost _ c2) = c1 <= c2

updatePQ :: (Ord k, Ord a) => [(k, a)]
         -> PQ.MinPQueue k a
         -> PQ.MinPQueue k a
updatePQ nodes pq = PQ.filterWithKey f pq `PQ.union` PQ.fromList nodes
  where ns = M.fromList $ map (\(x,y) -> (y,x)) nodes
...
graphMinDistance grid = f (PQ.singleton 0 (0,0)) M.empty
  where (_, fin) = A.bounds grid
        f priorityQ visited = if coord == fin
                                 then return cost
                                 else f (updatePQ  blah q) (M.insert coord True visited)
          where ((cost, coord), q) = PQ.deleteFindMin priorityQ
                neighbs = filter (\x -> isNothing (M.lookup x visited)) . U.getCrossNeighbCoords coord $ grid
