...
mapps = M.fromList [ (sort "acedgfb", 8)
                   , (sort "cdfbe", 5)
                   , (sort "gcdfa", 2)
                   , (sort "fbcad", 3)
                   , (sort "dab", 7)
...
findNumbers sevenSegments = do
  ...
  (three, fiveLengths) <- oneAndRest (sizeEqTo 2 . (one `Set.intersection`)) fiveLengths
  let zeroPattern = eight `Set.difference` three `Set.union` one

  (zero, sixLengths) <- oneAndRest (sizeEqTo 4 . Set.intersection zeroPattern) sixLengths
  (nine, sixLengths) <- oneAndRest (sizeEqTo 2 . Set.intersection one ) sixLengths

  (five, fiveLengths) <- oneAndRest (sizeEqTo 6 . Set.intersection nine . Set.union one) fiveLengths

  let six = head sixLengths
  let two = head fiveLengths
  Just $ M.fromList [ (zero, '0')
                    , (one, '1')
                    , (two, '2')
                    , (three, '3')
                    , (four, '4')
  ...
  where getByLengths x = filter ((== x) . Set.size) sevenSegments
